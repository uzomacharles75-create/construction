...
const StatCard = ({ icon: Icon, title, value, sub, iconBg, iconColor, isLoading }: any) => (
  <div className="bg-brand-navy-card border border-brand-border p-5 rounded-3xl border border-brand-border flex gap-4 shadow-sm hover:shadow-md transition-all">
    ...
    <div className="flex-1">
      <p className="text-[10px] font-black text-white/50 uppercase tracking-[0.15em] mb-1">{title}</p>
      {isLoading ? (
...
const QuickAccess = ({ icon: Icon, label, bg, path }: any) => (
  <Link to={path} className="flex flex-col items-center gap-2 cursor-pointer group shrink-0">
    ...
    <span className="text-[10px] font-black text-white/50 uppercase tracking-tighter group-hover:text-brand-yellow transition-colors">{label}</span>
  </Link>
);
...
const Dashboard = () => {
      ...
      <div className="max-w-[1600px] mx-auto pb-20">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <p className="text-xs text-white/50 font-bold uppercase tracking-widest mt-1 italic">
                {user?.company || "BuildHub Workspace"} • Premium Tier
            </p>
          </motion.div>
          
          <div className="flex items-center gap-3">
             <div className="flex items-center gap-2 bg-brand-navy-card px-5 py-2.5 rounded-2xl border border-brand-border text-[10px] font-black uppercase text-brand-muted shadow-sm">
               <MapPin size={14} className="text-brand-yellow" /> {new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
             </div>
             <Link 
                to="/dashboard/projects/new" 
                className="bg-brand-yellow text-brand-navy px-8 py-3 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-yellow hover:scale-[1.02] hover:scale-[1.02] transition-all flex items-center gap-2"
             >
  ...
  {/* QUICK ACCESS BAR */}
        <div className="bg-brand-navy-card border border-brand-border p-10 rounded-[3rem] border border-brand-border flex justify-between shadow-premium mb-10 overflow-x-auto no-scrollbar gap-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5 mb-10">
          <StatCard title="Active Sites" value={summary?.projectCount || '0'} sub="On-site operations" icon={Briefcase} iconBg="bg-brand-yellow-pale" iconColor="text-brand-yellow" isLoading={summaryLoading} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
          ...
          {/* Main Table Area */}
          <div className="lg:col-span-2 bg-brand-navy-card rounded-[2.5rem] border border-brand-border p-8 shadow-sm relative overflow-hidden">
            <div className="flex justify-between items-center mb-8">
              <h3 className="font-black text-white uppercase text-xs tracking-[0.2em]">Live Project Pulse</h3>
              <Link to="/dashboard/projects" className="text-[10px] font-black text-brand-yellow border-b-2 border-brand-yellow/10 hover:border-brand-yellow transition-all uppercase tracking-widest pb-1">View Portfolio</Link>
            </div>
                ...
                <tbody className="divide-y divide-slate-50">
                  {projectsLoading ? (
                    ...
                    <motion.tr 
                      ...
                      key={p._id} 
                      className="text-xs font-bold text-white/70 hover:bg-brand-navy-light transition-colors cursor-pointer group"
                    >
                      <td className="py-5 font-black text-white">{p.name}</td>
                      <td>
                        <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest ${
                          p.status === 'In Progress' ? 'bg-brand-yellow-pale text-brand-yellow' : 'bg-brand-navy-light text-brand-navy/50'
                        }`}>
                          {p.status}
                        </span>
                      </td>
                      <td>
                        <div className="w-32 h-1.5 bg-brand-navy-light rounded-full overflow-hidden">
                           <motion.div 
                              initial={{ width: 0 }} animate={{ width: `${p.progress}%` }}
                              className="h-full bg-brand-yellow shadow-[0_0_8px_rgba(37,99,235,0.4)]" 
                           />
                        </div>
                      </td>
          ...
          <div className="bg-brand-navy-card rounded-[2.5rem] border border-brand-border p-10 flex flex-col items-center shadow-sm">
            <h3 className="font-black text-white uppercase text-xs tracking-[0.2em] w-full mb-10">Financial Telemetry</h3>
            ...
            <div className="relative w-52 h-52 flex items-center justify-center mb-10">
               ...
               <div className="absolute flex flex-col items-center">
                 ...
                 <p className="text-[10px] text-white/50 font-black uppercase tracking-widest mt-1">Available</p>
               </div>
            </div>

            <div className="w-full space-y-5">
              <div className="flex justify-between items-center text-xs">
                <div className="flex items-center gap-3 font-bold text-white/50">
                    <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" /> 
                    <span className="uppercase tracking-widest text-[10px]">Total Revenue</span>
                </div>
                <span className="font-black text-white">${summary?.totalIncome?.toLocaleString() || '0'}</span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <div className="flex items-center gap-3 font-bold text-white/50">
                ...
                <span className="font-black text-white">${summary?.totalExpenses?.toLocaleString() || '0'}</span>
              </div>
